...
} from "@/app/helper/verify-webhook-signature";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isWebhookProcessed(webhook.event, webhook.payload.id)) {
      console.log(
        `Webhook already processed (retry): ${webhook.event}_${webhook.payload.id}`
      );
    ...
    try {
      ...
      console.log(
        `✅ Webhook processed successfully: ${webhook.event}_${webhook.payload.id}`
      );
    } catch (error) {
